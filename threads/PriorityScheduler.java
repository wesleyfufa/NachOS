...
public class PriorityScheduler extends Scheduler {
	...
	protected class PriorityQueue extends ThreadQueue {
		PriorityQueue(boolean transferPriority) {
			...
			// make comparision with effecttive priorities

			if (transferPriority == true) {
				this.priorityQ = new java.util.PriorityQueue<ThreadState>(11, new schedulerComp());
			}
			else{ //its gotta be false

				this.priorityQ = new java.util.PriorityQueue<ThreadState>(11);
			}
		}
		...
		public KThread nextThread() {
			...
			parent = priorityQ.poll();
			acquire((parent.getThread()));
		...
		public void addThread(ThreadState thread) {
			priorityQ.add(thread);
		}

		protected ThreadState pickNextThread() {
			// implement me
			// return null;
			return priorityQ.peek(); // priorityQ is already sorted so just first one
		}
		...
		public java.util.PriorityQueue<ThreadState> priorityQ;
		protected ThreadState parent = null;
	}
	...
	protected class ThreadState implements Comparable<ThreadState> {
		...
		public ThreadState(KThread thread) {
			...
			this.waitQueue = null;
			// [FIX]
			this.pqueues = new LinkedList<PriorityQueue>();
		...
		public int getEffectivePriority() {
			// implement me
			// state changed
			if ((priorityMinimum - 1) == priorityEff) {
				for (PriorityQueue pq : pqueues) {
					for (ThreadState ts : pq.priorityQ) {
						if (this != ts) {

							int priorityEffComp = ts.getEffectivePriority();
							if (ts.getEffectivePriority() > priorityEff) {
								priorityEff = ts.getEffectivePriority();
							}
						}
					}
				}
			...
			// if (priorityEff < priority) {
			// 	priorityEff = priority;
			// }
			return priorityEff = Math.max(priority, priorityEff);
			// return priorityEff;
		...
		public void renewPriority() {
			this.priorityEff = (priorityMinimum - 1);
		}

		// [FIX] [3]
		public void setPriority(int priority) {
			if (this.priority != priority) {
				this.priority = Math.min(Math.max(priorityMinimum, priority), priorityMaximum);
				renewPriority();
			}
		...
		public void acquire(PriorityQueue waitQueue) {
			// implement me
			pqueues.add(waitQueue);
		}
		public void removeQueue(PriorityQueue waitQueue) {
			pqueues.remove(waitQueue);
		}

		@Override
		public int compareTo(ThreadState ts) {
			return new Integer(ts.getPriority()).compareTo(this.getPriority());
		}

		// here
		// sentinal value for effective priority:
		protected int priorityEff = (priorityMinimum - 1);
		/** The thread with which this object is associated. */
		protected KThread thread;
		/** The priority of the associated thread. */
		protected int priority;

		// sorted data structure to store the the waiting threads:
		protected PriorityQueue waitQueue;
		//
		protected LinkedList<PriorityQueue> pqueues;

	}

	// special rules for thread comparsion

	public static class schedulerComp implements Comparator<ThreadState> {
		@Override
		public int compare(ThreadState a, ThreadState b) {
			return new Integer(b.getEffectivePriority()).compareTo(a.getEffectivePriority());
		}
	}
}